...
export const useSeasonBackupSettings = ({
  setError,
  setSuccessMessage,
}: SystemFeedback) => {
  ...
  const [isRenamingSeasonId, setIsRenamingSeasonId] = useState<string | null>(
    null,
  );
  const [isDeletingSeasonId, setIsDeletingSeasonId] = useState<string | null>(
    null,
  );
  ...
  const handleCreateSeason = async (event: React.FormEvent) => {
    ...
    if (
      !confirm(
        `确定要创建新赛季"${newSeasonName}"并将其直接设为活跃状态吗？\n\n此操作会重置球员的卡片数，但不会强行归档现有的其他活跃赛季。`,
      )
    )
      return;
  ...
  const handleUpdateSeasonStatus = async (
    id: string,
    ...
    if (
      !confirm(
        `确定要将该赛季的状态修改为【${nextStatus === "active" ? "活跃" : "已归档"}】吗？`,
      )
    )
      return;
    ...
    try {
      if (dryRun) {
          ...
          if (planned.length === 0) {
            ...
            alert(
              `【Retention Dry-run 检查结果】\n` +
                ...
                planned
                  .map((p, i) => `${i + 1}. ${p.filename} (${p.reason})`)
                  .join("\n"),
            );
          }
        }
        ...
        if (confirmInput !== "EXECUTE_RETENTION_DELETE") {
          if (confirmInput !== null)
            setError("确认文本输入错误，终止保留清理操作");
          return;
        }

        const res = await backupApi.cleanRetention(
          false,
          "EXECUTE_RETENTION_DELETE",
        );
        if (res.success) {
          setSuccessMessage(
            `保留策略物理清理已完成！成功清理 ${res.data.deletedCount} 个超出保存窗口的备份。`,
          );
          loadBackups();
        }
      }
  ...
  const handleRestore = async (backup: BackupDTO) => {
    const { key } = backup;
    if (backup.scope === "module") {
      setIsRestoring(key);
      setError(null);
      setSuccessMessage(null);
      try {
        const response = await backupApi.previewRestore(key);
        const preview = response.data;
        const rows = Object.values(preview.tableCounts).reduce(
          (sum, count) => sum + count,
          0,
        );
        const accepted = confirm(
          `【模块恢复 Preview】\n模块：${preview.module}\n策略：${preview.strategy}\n数据表：${Object.keys(preview.tableCounts).length}\n记录数：${rows}\n范围：${preview.selector.seasonId || "全模块"}\n\n${preview.warning}\n\n确认继续吗？`,
        );
        if (!accepted) return;
        if (!preview.canExecute)
          throw new Error("该模块恢复尚未在服务端环境变量中启用");
        const input = window.prompt(
          '请输入 "CONFIRM_MODULE_RESTORE" 执行模块恢复：',
        );
        if (input !== "CONFIRM_MODULE_RESTORE") {
          if (input !== null)
            throw new Error("二次确认文本错误，模块恢复已取消");
          return;
        }
        const result = await backupApi.restoreModule(
          key,
        ...
        if (result.success) {
          setSuccessMessage(
            result.message || `${preview.module} 模块恢复成功！`,
          );
          await loadBackups();
        }
      } catch (error) {
        console.error("模块恢复失败:", error);
        setError(
          error instanceof Error
            ? `模块恢复失败：${error.message}`
            : "模块恢复失败",
        );
      } finally {
        setIsRestoring(null);
    ...
    if (
      !confirm(
        "【警告】还原数据库将会删除并完全覆盖当前数据库中的所有球队、球员、赛程、进球和事件记录！此操作不可逆！\n\n确定要还原到选中的备份吗？",
      )
    )
      return;
